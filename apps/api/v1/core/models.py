...
class SupportInquiry(Base):
    ...
    message: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(String(20), default=SupportInquiryStatus.OPEN.value, index=True)
...
class PaymentTransaction(Base):
    ...
    periods_json: Mapped[list] = mapped_column("periods", JSON, default=list)
    contribution_ids_json: Mapped[list] = mapped_column("contribution_ids", JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
