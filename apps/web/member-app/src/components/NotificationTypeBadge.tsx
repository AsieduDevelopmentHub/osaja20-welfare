...
> = {
  ...
  welfare: { label: "Welfare", className: "bg-rose-50 text-rose-700", icon: Users },
  meeting: { label: "Meeting", className: "bg-slate-100 text-slate-700", icon: Bell },
};

export function NotificationTypeBadge({ type }: { type: string }) {
  const config = TYPE_CONFIG[type] ?? { label: type, className: "bg-slate-100 text-slate-600", icon: Bell };
  const Icon = config.icon;

  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${config.className}`}>
